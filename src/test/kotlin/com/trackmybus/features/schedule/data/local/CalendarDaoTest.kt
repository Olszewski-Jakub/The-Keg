...
class CalendarDaoTest : KoinTest {
    ...
    fun `Add calendar with existing ID`() {
        ...
        val calendarWithExistingId = Calendar("1", false, false, false, false, false, false, false, LocalDate(2024,1,1), LocalDate(2024,12,31))
    ...
    @Test
    fun `Get list of all calendars`() = runBlocking {
        val newCalendar = Calendar("1", true, true, true, true, true, true, true, LocalDate(2023,1,1), LocalDate(2023,12,31))
        runBlocking { calendarDao.addCalendar(newCalendar) }
    ...
    @Test
    fun `Get calendar with ID that does not exist`() = runBlocking {
    ...
    @Test
    fun `Update existing calendar`() = runBlocking {
        val newCalendar = Calendar("1", true, true, true, true, true, true, true, LocalDate(2023,1,1), LocalDate(2023,12,31))
        runBlocking { calendarDao.addCalendar(newCalendar) }
        val updatedCalendar = Calendar("1", false, false, false, false, false, false, false, LocalDate(2024,1,1), LocalDate(2024,12,31))
    ...
    @Test
    fun `Update calendar with non-existent ID`() = runBlocking {
        val updatedCalendar = Calendar("1", false, false, false, false, false, false, false, LocalDate(2024,1,1), LocalDate(2024,12,31))
        val result = calendarDao.updateCalendar(updatedCalendar)
    ...
    @Test
    fun `Delete calendar with non-existing ID`() = runBlocking {
