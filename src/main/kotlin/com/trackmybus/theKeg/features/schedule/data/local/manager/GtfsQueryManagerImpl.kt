package com.trackmybus.theKeg.features.schedule.data.local.manager
    ...
    companion object {
        ...
        private const val GET_STOPS_FOR_GIVEN_ROUTE_QUERY =
            ...
            WITH SelectedTrip AS (
                ...
                JOIN trips t ON st.trip_id = t.trip_id
                WHERE t.route_id = :routeId  -- Ensure the correct route
        ...
        private const val FIRST_AND_LAST_STOP_FOR_ROUTE_QUERY =
            ...
            JOIN stops s ON st.stop_id = s.stop_id
            JOIN trips t ON st.trip_id = t.trip_id
            WHERE t.route_id = :routeId  -- Replace '?' with the desired route_id
        ),
        MaxStopSequence AS (
            ...
            WHERE trip_id IN (
                SELECT trip_id
                FROM trips
                WHERE route_id = :routeId  -- Replace '?' with the desired route_id
            )
            GROUP BY trip_id
        )
        SELECT DISTINCT
