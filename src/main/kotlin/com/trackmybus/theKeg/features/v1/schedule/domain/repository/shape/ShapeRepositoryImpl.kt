package com.trackmybus.theKeg.features.v1.schedule.domain.repository.shape
...
class ShapeRepositoryImpl(
    ...
    override suspend fun getByShapeId(shapeId: String): Result<List<Shape>> {
        logger.info("Fetching shapes by shape ID: $shapeId")
        return shapeDao
            .getShapesByShapeId(shapeId)
            .mapResult { it.map { shapeEntity -> shapeEntity.toModel() } }
            .also { result ->
                result.onSuccess { logger.info("Successfully fetched shapes by shape ID: $shapeId") }
                result.onFailure { logger.error("Error fetching shapes by shape ID: $shapeId", it) }
            }
    }
}
